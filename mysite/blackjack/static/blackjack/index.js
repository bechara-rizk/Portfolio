    ...
    for (let i = 0; i < renderHand; i++) {
        ...
        for (let j = 0; j < splitCards[i].length / 2; j++) {
            if ((/[a-zA-Z]/).test(splitCards[i][splitCards[i].length - 2 - j * 2])) {
                cardTag += '<img class="card" src="/static/blackjack/img/cardSet/' + splitCards[i][splitCards[i].length - 1 - j * 2] + '/' + splitCards[i][splitCards[i].length - 2 - j * 2][0] + '.png" alt="">';
            } else {
                cardTag += '<img class="card" src="/static/blackjack/img/cardSet/' + splitCards[i][splitCards[i].length - 1 - j * 2] + '/' + splitCards[i][splitCards[i].length - 2 - j * 2] + '.png" alt="">';
            }
        }
        let idToAdd = (i + 1);
...
function renderGame() {
    cardTag = '<img class="card" src="/static/blackjack/img/cardSet/' + cards[cards.length - 1] + '/' + cards[cards.length - 2] + '.png" alt="">';
